const defaultLocales = {
  en: {
    ...
    chat_limit: 'Only the most recent 100 messages are kept. To view the full chat history, you can join the corresponding Mixin community.',
    ...
    hello_desc: 'Pando lot , a messaging platform for users to simply and instantly message each other. Before we start, here are some rules you should be aware of:',
  ...
  zh: {
    ...
    chat_limit: '只保留最近的100条信息。要查看完整的聊天历史，你可以加入相应的Mixin社区。',
    chat_placeholder: '回复',
    disconnect_wallet: '断开钱包连接',
    connect_title: '连接钱包',
    connect_subtitle: '选择可用的钱包之一。',
    hello_title: '欢迎来到 Pando Lots',
    hello_desc: 'Pando lot ，是一个供用户简单、即时地相互留言的消息平台。在我们开始之前，这里有一些你应该注意的规则:',
  ...
  ja: {
    ...
    chat_limit: '直近の100件のメッセージのみが保存されます。全てのチャット履歴を見るには、対応する Mixin コミュニティに参加することができます。',
    chat_placeholder: '返信',
    disconnect_wallet: 'ウォレットを切断',
    connect_title: 'コンセトウォレット',
    connect_subtitle: 'ウォレットプロバイダーと接続する。',
    hello_title: 'ようこそ Pando Lots',
    hello_desc: 'Pando lot , ユーザーが簡単かつ即座にメッセージをやり取りできるメッセージングプラットフォームです。始める前に、いくつか知っておくべきルールを紹介します:',
